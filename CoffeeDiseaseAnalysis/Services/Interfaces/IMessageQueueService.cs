// File: CoffeeDiseaseAnalysis/Services/Interfaces/IMessageQueueService.cs
...
// File: CoffeeDiseaseAnalysis/Services/MessageQueueService.cs
    ...
    public class MessageQueueService : IMessageQueueService, IDisposable
    {
        ...
        private async Task ProcessImageAsync(BasicDeliverEventArgs ea)
        {
            ...
            catch (Exception ex)
            {
                ...
                // Reject message và không requeue nếu đã thử nhiều lần
                var requeue = ea.BasicProperties?.Headers?.ContainsKey("x-retry-count") != true;
                _channel?.BasicNack(deliveryTag, false, requeue);

                // Update prediction log nếu có
                try
                {
                    using var scope = _serviceProvider.CreateScope();
                    var context = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
                    ...
                    if (request != null)
                    {
                        ...
                        if (predictionLog != null)
                        {
                            ...
                            predictionLog.ErrorMessage = ex.Message;
                            predictionLog.ResponseTime = DateTime.UtcNow;
                            await context.SaveChangesAsync();
                        }
                    }
